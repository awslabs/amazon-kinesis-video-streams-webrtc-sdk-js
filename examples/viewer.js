...
async function startViewer(localView, remoteView1, remoteView2, formValues, onStatsReport, onRemoteDataMessage) {
  ...
  viewer.signalingClient.on('open', async () => {
    ...
    if (formValues.sendVideo || formValues.sendAudio) {
      try {
        ...
        viewer.peerConnection.addTransceiver('video', {'direction': 'recvonly', streams: [viewer.localStream]});
  ...
  viewer.peerConnection.addEventListener('track', event => {
    ...
    if (remoteView1.srcObject) {
      return;
    }
    viewer.remoteStream = event.streams[0];

    let i = 0;
    for (const track of viewer.remoteStream.getVideoTracks()) {
      if (i === 0) {
        remoteView1.srcObject = new MediaStream([track, viewer.remoteStream.getAudioTracks()[0]]);
      } else {
        remoteView2.srcObject = new MediaStream([track]);
      }
      i++;
    }
