...
async function startMaster(localView, remoteView, formValues, onStatsReport, onRemoteDataMessage) {
    ...
    try {
        ...
        master.channelHelper = new ChannelHelper(
            ...
            formValues.endpoint,
            $('#master-heading').text() === 'Viewer' ? KVSWebRTC.Role.VIEWER : KVSWebRTC.Role.MASTER,
            ingestionMode,
            `[${ROLE}]`,
            $('#master-heading').text() === 'Viewer' ? formValues.clientId || randomString() : undefined,
        );
...
registerMasterSignalingClientCallbacks = (signalingClient, formValues, configuration, onStatsReport, onRemoteDataMessage) => {
    ...
    signalingClient.on('open', async () => {
        ...
        if (formValues.showJSSButton) {
            ...
            console.log(`[${role}] Waiting for media ingestion and storage viewer to join...`);
    ...
    signalingClient.on('sdpOffer', async (offer, remoteClientId) => {
        printSignalingLog(`[${role}] Received SDP offer from client`, remoteClientId);
        ...
        answerer.getPeerConnection().addEventListener('connectionstatechange', async event => {
            ...
            if (master.channelHelper.isIngestionEnabled() && event.target.connectionState === 'connected') {
                console.log(
                    `[${role}] Successfully joined the storage session. Media is being recorded to`,
...
function stopMaster() {
    const role = ROLE;
    try {
        console.log(`[${role}] Stopping master connection`);
        master.sdpOfferReceived = true;
...
async function connectToMediaServer(masterRunId) {
    ...
    if (success) {
        ...
        console.error(`[${role}]`, 'Error joining storage session');
